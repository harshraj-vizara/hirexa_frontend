...
(function () {
  ...
  var TEMPLATE = [
    ...
    '  <svg class="hxa-userdd-chev" width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="#24282B" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><polyline points="6 9 12 15 18 9"/></svg>',
    "</button>",
  ...
  function titleCase(str) {
    return String(str || "")
      .toLowerCase()
      .replace(/(^|[\s\-'])([a-zÀ-ɏ])/g, function (m, sep, ch) {
  ...
  function mount(anchor) {
    ...
    if (completeBtn) {
      completeBtn.addEventListener("click", function () {
        ...
        try {
          localStorage.setItem("openCompleteSetup", "1");
        } catch (e) {}
        window.location.href = "/b4kx";
      });
    }
  ...
  function avatarEls() {
    return document.querySelectorAll(
      ".hxa-userdd-avatar, #user-avatar, .user-avatar",
    );
  }
  function applyCompanyLogo() {
    ...
    var cached = null;
    try {
      cached = localStorage.getItem("hirexaCompanyLogo");
    } catch (e) {}
    if (cached)
      for (var i = 0; i < els.length; i++) applyLogoToEl(els[i], cached);
    ...
    fetch("/api/company/logo?email=" + encodeURIComponent(user.email))
      ...
      .then(function (d) {
        ...
        if (path) {
          try {
            localStorage.setItem("hirexaCompanyLogo", path);
          } catch (e) {}
  ...
  window.HXAUserDropdown = {
    mount: mount,
    init: init,
    applyCompanyLogo: applyCompanyLogo,
  };
})();
